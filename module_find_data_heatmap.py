...
def find_thresholds_heatmap( input_params, rv_params, OU_params=None ):
    ...
    https://quantresearch.org/OTR.py.t*** Now analysinxt and in Chapter 13 of his book "Advances in financial machine learning".
    ...
    :param list_profit_taking: list of real numbers which are the values to analyse for the upper threshold (profit-taking)
    :param list_stop_loss: list of real numbers which are the values to analyse for the lower threshold (stop-loss)
    :param min_num_trials: minimum number of iterations for the determination of the optimal threshold. Take a high number and check
    the convergence of the outputwrt to this parameter.
    :param max_horizon: maximum number of periods (discrete times in the Ornstein-Uhlenbeck equation above) before the position (investment) is closed.
    ...
    if (input_params.evolution_type == "Ornstein-Uhlenbeck_equation"):
        for en in input_params.list_enter_value:
            ...
            #for mh,pt,sl in product( input_params.list_max_horizon, adpt_list_profit_taking, adpt_list_stop_loss ):
