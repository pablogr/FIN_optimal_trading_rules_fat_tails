...
# Set to True to perform just one attempt to fit to Levy stable distribution (instead of several ones)
efficient_fit_to_levy_stable = True
...
n_random_trials = 14 #  Suggested value for levy_stable: 2. For oos with t-student, min 11.
...
max_n_iter = 20 # Suggested value for levy_stable: 1. For oos with t-student, min 16.
...
# Warning: If you use both positive and negative values for list_enter_value in a single run, the code will give error when plotting heatmaps. Hence you should use something like arange(-0.02500000001, -0.01000000000, 0.001) .
#          However, if you use just positive or just negative values in an out-of-sample run it will also give an error. Hence you should use something like array( list( arange(-0.02500000001, -0.01000000000, 0.001) ) +  list( arange(0.010000000, 0.025000002, 0.001) ) ) .
list_enter_value   = array( list( arange(-0.02500000001, -0.01000000000, 0.001) ) +  list( arange(0.010000000, 0.025000002, 0.001) ) )  # arange(-0.02500000001, -0.01000000000, 0.001) # array( list( arange(-0.02500000001, -0.01000000000, 0.001) ) +  list( arange(0.010000000, 0.025000002, 0.001) ) ) #arange(-0.08,0.08,0.01)   #arange(-0.8,-0.7,0.2) #array([-2]) # array([-3,-2,-1.8,-1.6,-1.4,-1.2,-1.0,-0.9,-0.8,-0.7,-0.6,-0.5,-0.4,-0.3,-0.2,-0.1,-0.0000001]) #array([-0.8]) #arange(-2.001,0.05,0.1) #array([-0.8]) # array([-6.0,-4.0,-2.0,0]) array([-4.0,-2.0,-1.0,-0.75,-0.5,0])
...
# Number of Monte Carlo path generated in the evaluation of each set of trading rules. Recommended value: 10000 for out-of-sample calculations. This quantity is central for the numerical complexity of the calculations and for their accuracy. Set it carefully.
...
# First date for the out-of-sample calculation. E.g. credit_cards: 2019-11-30; Other stocks: '2018-11-30';  crypto: '2022-05-01'
first_oos_date =  '2019-11-30'

# Last date for the out-of-sample calculation. E.g. stocks: '2022-11-29'; crypto: '2022-12-20'  #
...
in_sample_recalibration_freq = '3m' #'3m'
